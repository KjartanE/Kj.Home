...
export class Geometry {
  ...
  /**
   * Creates a point at a given position
   */
  createPoint(point: Point, options: GeometryOptions = {}): THREE.Mesh {
  ...
  createCircle(center: Point, radius: number, options: GeometryOptions = {}): THREE.Line {
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      points.push(new THREE.Vector3(center.x, center.y, center.z || 0));
    }
    ...
    this.animationStates.set(rectangle, {
      ...
      update: (progress: number) => {
        ...
        if (currentSegment < 4) {
          ...
          const lerpedPoint = new THREE.Vector3().lerpVectors(startPoint, endPoint, pointProgress);
