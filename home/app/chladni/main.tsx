...
export class Particle {
  ...
  update(n, m) {
    ...
    if (amount >= 0) {
      if (this.chladni(this.x + vibrationmax, this.y, n, m) >= amount) {
        this.x = this.constrain(this.x - randomNum * amount * this.speed + vibrationX, -1, 1);
      } else {
        this.x = this.constrain(this.x + randomNum * amount * this.speed + vibrationX, -1, 1);
      }
      if (this.chladni(this.x, this.y + vibrationmax, n, m) >= amount) {
        this.y = this.constrain(this.y - randomNum * amount * this.speed + vibrationY, -1, 1);
      } else {
        this.y = this.constrain(this.y + randomNum * amount * this.speed + vibrationY, -1, 1);
      }
    } else {
  ...
  display() {
    ...
    dotGeometry.setAttribute("position", new THREE.BufferAttribute(new Float32Array([this.x, this.y, 0]), 3));
