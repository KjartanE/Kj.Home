...
class DotMatrix {
  ...
  constructor(
    ...
    this.matrix = Array.from({ length: this.matrix_x_size }, () =>
      Array.from({ length: this.matrix_y_size }, () =>
        Array.from(
          { length: this.matrix_z_size },
          () => new MatrixVertex(0, 0, 0)
        )
      )
    )
    ...
    for (let x = 0; x < this.matrix_x_size; x++) {
      for (let y = 0; y < this.matrix_y_size; y++) {
        for (let z = 0; z < this.matrix_z_size; z++) {
          ...
          const new_vertex: MatrixVertex = new MatrixVertex(
            x * this.density,
            y * this.density,
            z * this.density
          )
    ...
    for (let x = 0; x < this.matrix_x_size; x += 1) {
      for (let y = 0; y < this.matrix_y_size; y += 1) {
        for (let z = 0; z < this.matrix_z_size; z += 1) {
          ...
          if (
            x < this.prev_matrix.length &&
            y < this.prev_matrix[x].length &&
            z < this.prev_matrix[x][y].length
          ) {
            let current_vertex: MatrixVertex = this.prev_matrix[x][y][z]
          ...
          const new_vertex = new MatrixVertex(
            x * this.density,
            y * this.density,
            z * this.density
          )
  ...
  update_density(new_density: number) {
    ...
    this.matrix = new MatrixVertex[this.matrix_x_size][this.matrix_y_size][
      this.matrix_z_size
    ]()
